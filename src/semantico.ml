...
let rec verifica_op t1 t2 op ls =
    match ls with
    (r,a1,a2)::ls -> if ((t1 == a1) && (t2 == a2)) then r
                     else verifica_op t1 t2 op ls
    | [] -> print_endline ("Expressao invalida. Operador " ^ get_nome op ^" usado indevidamente.\nParametros arg1: " ^ get_nome_tipo t1^ " e arg2: "^ get_nome_tipo t2); failwith "Erro de tipos no operador" 
...
let rec tipo_retorno amb expr current param = 
    let entrada = Hashtbl.find amb current in
        ...
        | ExpBin (op,e1,e2) ->
            ignore(tipo_retorno amb e1 current param); (*ver se o ignore nao estragou alguma coisa*)
...
and verifica_cmd amb cmd current param =
    ...
    | CmdIf (e, ce, cs) -> 
        ...
        (if (e.tipo == TGen) then
                        erro "comando if " e.pos " variavel nao inicializada");
        begin      	
